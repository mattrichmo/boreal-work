...
import { loadGlobalOverview, loadGlobalProjects, loadGlobalQueues, loadRepoRollup, loadRepoSprintBoard, loadRepoTaskDetail, type GlobalOverviewBody, type RepoSprintBoardBody, type RepoTaskDetailBody } from "./loaders.js";
import { visibleRollupRows, type RollupDisclosureState } from "./routes/rollup.js";
...
export function activeRowIds(body: RouteBody | undefined, filters?: TuiFilterState, expandedIds?: RollupDisclosureState): readonly string[] {
  if (!body) return [];
  switch (body.kind) {
    case "repo.rollup": return visibleRollupRows(body.value, filters, expandedIds).map((row) => row.id);
