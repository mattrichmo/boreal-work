...
export function FreshnessLine({ generatedAt, label, filter, error, stale, warnings, blocked, width }: {
  readonly generatedAt?: string; readonly label: string; readonly filter?: string; readonly error?: string; readonly stale: boolean; readonly warnings: number; readonly blocked: boolean; readonly width: number;
}) {
  ...
  const text = error ? `${generatedAt ? "Last known data · " : ""}${error.replaceAll("\n", " ")} · r retry`
    : [label, filter ? `view: ${filter}` : "", stale ? "STALE" : "", warnings ? `${warnings} warnings · ? details` : "", blocked ? "read-only" : "", updatedAt === undefined ? "Loading" : `updated ${updatedAt}`].filter(Boolean).join(" · ");
  return <Text color={error ? COLOR.danger : stale || warnings ? COLOR.warn : COLOR.faint} wrap="truncate">{fit(text, width)}</Text>;
}
