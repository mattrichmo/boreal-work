---
id: boreal.workflow.add-memory.v1
title: Add Memory
group: 20-memory
status: v1
risk: medium
...
Create new durable memory when no existing record already covers the fact.

## When To Use

Use this workflow when the user's request requires create new durable memory when no existing record already covers the fact. Do not use it for adjacent work when a narrower workflow exists.
