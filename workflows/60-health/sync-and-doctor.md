---
id: boreal.workflow.sync-and-doctor.v1
title: Sync And Doctor
group: 60-health
status: v1
risk: medium
writes_state: true
...
Check workspace health and run safe repairs only when explicit.

## When To Use

Use this workflow when the user's request requires check workspace health and run safe repairs only when explicit. Do not use it for adjacent work when a narrower workflow exists.
