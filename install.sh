...
# published yet, explicit --from-source or --allow-source-fallback builds the
...
BACKUP_ROOT=""
ROLLBACK_NEEDED=0
UI_MODE="${BOREAL_INSTALL_UI:-auto}"
INSTALL_TUI="${BOREAL_INSTALL_TUI:-1}"
VERIFY_INSTALL="${BOREAL_VERIFY_INSTALL:-1}"
REPLACE_EXISTING=0
SOURCE_LOCKED=0
...
Usage: install.sh [--version VERSION] [--prefix PATH] [--archive PATH]
                  [--from-source] [--ref REF] [--interactive | --yes | --plain]
                  [--no-dashboard] [--no-verify] [--replace-existing]
...
script again stages and replaces the selected components, with rollback on failure.

With a supported Node.js and a terminal, installation opens a multi-screen
wizard. --yes/--non-interactive and --plain never prompt. A piped bootstrap
reads wizard keys from /dev/tty, not the script's standard input.

Source builds are explicit: use --from-source or --allow-source-fallback.
...
# Only the embedded wizard executes. No companion executable is fetched from a
# mutable remote URL. UI output goes to /dev/tty; result JSON is never eval'd.
case "$UI_MODE" in auto|interactive|plain|0) ;; *) die "BOREAL_INSTALL_UI must be auto, interactive, or plain" ;; esac
case "$INSTALL_TUI:$VERIFY_INSTALL:$REPLACE_EXISTING:$ALLOW_SOURCE_FALLBACK" in *[!01:]*) die "installer toggles must be 0 or 1" ;; esac
HAS_NODE=0
if command -v node >/dev/null 2>&1 && node -e 'const n=+process.versions.node.split(".")[0]; process.exit(n>=20&&n<27?0:1)' >/dev/null 2>&1; then HAS_NODE=1; fi
HAS_TTY=0
if [ -t 1 ] && [ "${TERM:-}" != dumb ] && [ -z "${CI:-}" ] && ( : </dev/tty ) 2>/dev/null; then HAS_TTY=1; fi
if [ "$UI_MODE" = interactive ] && { [ "$HAS_NODE" -ne 1 ] || [ "$HAS_TTY" -ne 1 ]; }; then
  die "interactive installation requires Node.js 20–26 and a terminal; use --yes for a plain install"
fi
if { [ "$UI_MODE" = auto ] || [ "$UI_MODE" = interactive ]; } && [ "$HAS_NODE" -eq 1 ] && [ "$HAS_TTY" -eq 1 ]; then
  UI_TEMP=$(mktemp -d "${TMPDIR:-/tmp}/boreal-install-ui.XXXXXX")
  trap 'rm -rf "$UI_TEMP"' EXIT
  trap 'exit 130' INT
  trap 'exit 143' TERM HUP
# BOREAL_EMBEDDED_WIZARD_BEGIN
cat > "$UI_TEMP/wizard.cjs" <<'BOREAL_WIZARD_JS'
...
BOREAL_WIZARD_JS
# BOREAL_EMBEDDED_WIZARD_END
  if node "$UI_TEMP/wizard.cjs" machine "$PREFIX" "$REQUESTED_VERSION" "$SOURCE_INSTALL" "$LOCAL_ARCHIVE" "$SOURCE_REF" "$INSTALL_TUI" "$VERIFY_INSTALL" "$REPLACE_EXISTING" "$SOURCE_LOCKED" > "$UI_TEMP/result.json"; then
    PREFIX=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).prefix' "$UI_TEMP/result.json")
    selected_source=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).source' "$UI_TEMP/result.json")
    INSTALL_TUI=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).dashboard?1:0' "$UI_TEMP/result.json")
    VERIFY_INSTALL=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).verify?1:0' "$UI_TEMP/result.json")
    REPLACE_EXISTING=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).replace_existing?1:0' "$UI_TEMP/result.json")
    if [ "$selected_source" = source ]; then SOURCE_INSTALL=1; else SOURCE_INSTALL=0; fi
  else
    status=$?
    exit "$status"
  fi
...
phase() { printf '\n  [%s/4] %s\n' "$1" "$2"; }
printf '\n  BOREAL / WORK\n  Machine installation · project data stays untouched\n'
phase 1 "Resolve installation source"
...
bootstrap_from_source() {
  ...
  trap 'rm -rf "$source_temp" "$source_output"' EXIT
  trap 'exit 130' INT
  trap 'exit 143' HUP TERM
  ...
  set +e
  set -- --archive "$source_archive" --prefix "$PREFIX" --non-interactive
  [ "$INSTALL_TUI" -eq 1 ] || set -- "$@" --no-dashboard
  [ "$VERIFY_INSTALL" -eq 1 ] || set -- "$@" --no-verify
  [ "$REPLACE_EXISTING" -eq 0 ] || set -- "$@" --replace-existing
...
case "$PREFIX" in
  ""|/|*/../*|*/..|..)
    die "refusing an empty, root, or parent-traversing installation prefix"
    ;;
  /*) ;;
...
if printf '%s' "$PREFIX" | LC_ALL=C grep '[[:cntrl:]]' >/dev/null 2>&1; then
  die "installation prefix must not contain control characters"
fi
if [ -d "$PREFIX" ] && [ "$(CDPATH= cd -- "$PREFIX" && pwd -P)" = / ]; then
  die "installation prefix resolves to the filesystem root"
fi

if { [ -e "$PREFIX/bin/bwrk" ] || [ -L "$PREFIX/bin/bwrk" ]; }   && [ ! -f "$PREFIX/share/boreal/release.json" ] && [ "$REPLACE_EXISTING" -ne 1 ]; then
  die "unrecognized bwrk already exists at $PREFIX/bin/bwrk; use a separate --prefix to retain v1, or explicitly --replace-existing"
fi
...
if [ -n "$LOCAL_ARCHIVE" ]; then
  ...
  if [ -z "$REQUESTED_VERSION" ]; then
    ...
    if ! latest_url=$(curl -fsSL -o /dev/null -w '%{url_effective}' "https://github.com/$REPOSITORY/releases/latest"); then
      if [ "$ALLOW_SOURCE_FALLBACK" -eq 1 ]; then bootstrap_from_source; fi
      die "cannot resolve a release; choose --from-source explicitly or retry the release download"
    fi
...
if [ "$AUTO_SELECTED_RELEASE" -eq 1 ]; then
  ...
  if ! curl -fsSL -o /dev/null "$RELEASE_URL/$ARCHIVE_NAME" \
    ...
    if [ "$ALLOW_SOURCE_FALLBACK" -eq 1 ]; then bootstrap_from_source; fi
    die "no compatible v2 release for ${REQUESTED_VERSION}; use --from-source explicitly (no automatic source build was started)"
  fi
fi
...
# Every destination has independent rollback markers. A failure while backing
# up one file must never delete a later file which has not yet been backed up.
rollback_item() {
  relative=$1
...
# Reject links and special members before extracting; checking after extraction
# is too late to prevent an archive symlink from redirecting subsequent files.
LC_ALL=C tar -tvzf "$ARCHIVE_PATH" > "$TEMP_ROOT/archive.types" || die "cannot inspect archive member types"
awk 'substr($0,1,1) != "-" && substr($0,1,1) != "d" { bad=1 } END { exit bad }' "$TEMP_ROOT/archive.types"   || die "release archive must contain only regular files and directories (no links or special files)"
...
publish_item() {
  ...
  if [ -e "$PREFIX/$relative" ] || [ -L "$PREFIX/$relative" ]; then
    mv "$PREFIX/$relative" "$BACKUP_ROOT/$relative"
  fi
  if [ -e "$INSTALL_STAGE/$relative" ]; then
    # Write intent before rename so a signal cannot hide a published file.
    : > "$BACKUP_ROOT/.published.$marker"
    mv "$INSTALL_STAGE/$relative" "$PREFIX/$relative"
  fi
}
