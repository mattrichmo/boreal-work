...
- The global project registry foundation is now a machine-local `boreal.project-registry.v1` document at the resolved registry root, with validation that rejects cross-project path leakage.
- `bwrk registry list`, `registry add`, `registry remove`, `registry import-setup`, and `registry doctor` now manage explicit project rows, idempotently seed the current workspace from `.boreal/project.json`, and detect stale roots, moved projects, memory-root drift, and project setup mismatches.
