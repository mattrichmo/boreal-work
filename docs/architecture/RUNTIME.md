...
JSONL ledgers are a bridge toward Git-native collaboration, not a second runtime backend yet. `export ledgers` writes one deterministic JSONL file per snapshot section plus a hash manifest under `.boreal/ledgers`; `import ledgers` verifies the manifest, per-file counts, per-file hashes, schemas, and references before merging records through the same conflict rules as JSON import. `ledger status` and `doctor` report drift when an existing ledger export no longer matches the current runtime state.
