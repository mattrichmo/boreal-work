...
Duplicate handling starts with read-only detection. `duplicate scan` groups likely duplicate work, raw source, and wiki records and emits manual-review merge plans. `merge plan` prints the same non-destructive plan shape without applying changes. `compact analyze` identifies old closed work and vault page compaction candidates, then emits review-only plans that preserve evidence IDs, verification IDs, source refs, wiki links, and original paths. Actual merge and compaction apply commands must stay explicit, reviewable, and source-preserving.
