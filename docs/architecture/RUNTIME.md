...
The repo-local `memory/` vault is the durable Magico-style project memory scaffold. `vault init` creates canonical `raw`, `wiki`, `work`, `graph`, `ledgers`, `dashboards`, and `memory/.boreal` paths plus Markdown index pages and JSONL placeholders. `raw add` appends immutable source records to `memory/raw/index.jsonl`; vault JSONL appends are serialized with repo-local locks under `memory/.boreal/locks/`. `wiki create` writes flat-frontmatter Markdown pages under `memory/wiki`. Runtime code still treats the vault as an adapter boundary; doctor validates the scaffold, raw JSONL, wikilinks, source refs, orphan pages, and stale claim markers, but domain rules stay in the engine.
